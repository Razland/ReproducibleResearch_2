---
title: Some Health and Economic Effects of Historic Weather Events
output:
  html_document: 
    fig_caption: yes
    toc: yes
...
library(ggplot2)
library(grid)
source("multiplot.R")                       ## Helper file from "The R Cookbook"
                                            ## see file comments for url.
...
```{r plotRangeDat, cache = TRUE, comment = NA}
##require(dplyr)
sumInj <- 
...
```{r reportInjuries, cache = TRUE, comment = NA }
...
```{r reportDeaths, cache = TRUE, comment = NA}
mutate(sumFatal[1:3, ],
...
```{r multiPlot, echo = TRUE, cache = TRUE}
...
```{r q1Cleanup, echo = FALSE}
## Echo off -- memory management not pertinent to report processing.
rm(sumFatal,  sumInj, sumFatal92,           ## Explicitly cleanup Q1 temp data 
   sumInj92, convertEvent, targFileName,  
   loadStormDat, downLoadData, 
   insertMark, cleanStormDat, P1, P2, P3, P4, 
   evCl)
```        
...
```{r cleanExpnt, cache = TRUE, comment = NA}
...
```{r compCropPropDamage, cache = TRUE, comment = NA}
costDat <- 
...
```{r reportPropCropDmg, comment = NA}
propDat
```    

The top 6 causes of crop damage are:

```{r reportCropDmg, comment = NA}
cropDat
```    
Panel plot the top property and crop damage-causing weather events.  

```{r plotPropCropDmg, comment = NA}
multiplot(P5, P6, cols = 2)
```     

```{r cleanStuff, comment = NA, echo = FALSE}
rm(costDat, dmgN)
```
...
```{r projectCleanup, echo = FALSE, comment = NA}
## Echo off -- memory managment step not pertinent to analyses
rm(propDat, cropDat, P5, P6,                ## Explicit cleanup environment. 
   stormDat, chainGsub, cleanExpnt) 
```