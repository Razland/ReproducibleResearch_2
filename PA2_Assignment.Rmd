---
title: Some Health and Economic Impacts of Historic Weather Events
output:
  html_document: 
    fig_width: 7
    fig_height: 6
...
```{r multiPlot, echo = TRUE, cache = TRUE, comment = NA }
...
```{r reportPropCropDmg, comment = NA, cache = TRUE }
propDat[1:3, ]
```    

The top 3 causes of crop damage are:

```{r reportCropDmg, comment = NA, cache = TRUE }
cropDat[1:3, ]
```    
...
```{r plotPropCropDmg, comment = NA, cache = TRUE }
...
rm(propDat, cropDat, P5, P6,                ## Explicit cleanup environment. 
   ...
   costDat, dmgN, eventVect, multiplot)
```
